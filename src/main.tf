...
module "vm-yc" {
  source              = "git::https://github.com/DioRoman/ter-final.git//src/modules/yandex-vm?ref=main"
  vm_name             = var.vm-yc[0].instance_name 
  vm_count            = var.vm-yc[0].instance_count
  zone                = var.vpc_default_zone[0]
  subnet_ids          = module.yandex-vpc.subnet_ids
  image_id            = data.yandex_compute_image.ubuntu.id
  platform_id         = var.vm-yc[0].platform_id
  cores               = var.vm-yc[0].cores
  memory              = var.vm-yc[0].memory
  disk_size           = var.vm-yc[0].disk_size 
  public_ip           = var.vm-yc[0].public_ip
  security_group_ids  = [module.yandex-vpc.security_group_ids["web"]]
  
  labels = {
    env  = var.vm-yc[0].env_name
    role = var.vm-yc[0].role
  }
